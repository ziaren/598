...
#df_all <- final_tidy_data
#df <- final_tidy_data_complete

#hotfix for HHI- ONLY RUN ONCE
...
diff_df <- rbind(filter(tmt_df, redcap_event_name == "session_2_arm_1"),filter(wl_df, redcap_event_name == "session_3_arm_1"))

```

```{r difference-difference models}
...
```{r difference-baseline models}
#filter(df, df$`Treatment or Waitlist` == "T")
diff_tmt <- tmt_df %>% filter(redcap_event_name == "session_2_arm_1") %>% select(record_id, diff_EC:diff_HHI)
base_tmt <- inner_join(filter(df, redcap_event_name == "session_1_arm_1" & `Treatment.or.Waitlist` == "T"), diff_tmt, by = "record_id")

diff_wl <- wl_df %>% filter(redcap_event_name == "session_3_arm_1") %>% select(record_id, diff_EC:diff_HHI)
base_wl <- inner_join(filter(df, redcap_event_name == "session_2_arm_1" & `Treatment.or.Waitlist` == "W"), diff_wl, by = "record_id")

base_diff_df <- rbind(base_tmt, base_wl)


fit_base_aphab_both <- lm(diff_global ~ susceptibility + severity + benefits + barriers + cues_action + efficacy + HHI + global, data = base_diff_df)
summary(fit_base_aphab_both)

fit_base_aphab_other <- lm(diff_global ~ susceptibility + severity + benefits + barriers + cues_action + efficacy + HHI, data = base_diff_df)
summary(fit_base_aphab_other)

fit_base_aphab_same <- lm(diff_global ~ susceptibility + severity + benefits + barriers + cues_action + efficacy + global, data = base_diff_df)
summary(fit_base_aphab_same)

fit_base_HHI_both <- lm(diff_HHI ~ susceptibility + severity + benefits + barriers + cues_action + efficacy + HHI + global, data = base_diff_df)
summary(fit_base_HHI_both)

fit_base_HHI_other <- lm(diff_HHI ~ susceptibility + severity + benefits + barriers + cues_action + efficacy + global, data = base_diff_df)
summary(fit_base_HHI_other)

fit_base_HHI_same <- lm(diff_HHI ~ susceptibility + severity + benefits + barriers + cues_action + efficacy + HHI, data = base_diff_df)
summary(fit_base_HHI_same)
